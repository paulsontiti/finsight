...
export class SaveUserSessionUseCase implements UseCase<
  SaveUserSessionInput,
  UserSession
> {
  constructor(private userSessionRepo: PrismaUserSessionRepository) {}

  async execute(input: SaveUserSessionInput) {
    const existingSession = await this.userSessionRepo.findByUserDevice(
      input.userId,
      input.deviceId,
    );
    ...
    if (existingSession) {
      return this.userSessionRepo.updateLastActive(existingSession.id);
    }
